...
if (!empty($forum)) {      // User is starting a new discussion in a forum
    ...
    if ($prunemform->is_cancelled()) {
        ...
        $newdiscussion->usermodified = (forum_is_anonymous($forum->id)) ? $discussion->userid : $post->userid; // UDE-HACK
...
// UDE-HACK
if (!forum_is_anonymous($forum->id)) {
...
if ($mform_post->is_cancelled()) {
    ...
    if ($fromform->edit) {           // Updating a post
		...
		if (forum_is_anonymous($forum->id) && !has_capability('moodle/course:manageactivities', context_course::instance($forum->course) , $USER->id, false)) {
			/* get user-ID of Anonymous user */
			$addpost->userid = get_config(null, 'forum_anonuser_id');
			$noDisplay = true;
		} else {
		...
		// UDE-HACK: do not store group id for anonymous postings
		if (forum_is_anonymous($forum->id) && !has_capability('moodle/course:manageactivities', context_course::instance($forum->course) , $USER->id, false)) {
			$fromform->groupid = -1;
		}
		// UDE-HACK end
        ...
        if ($completion->is_enabled($cm) &&
			...
			if ($noDisplay)
	      		$timemessage = 0; //no email-messageformat
// 			redirect(forum_go_back_to("view.php?f=$fromform->forum"), $message.$subscribemessage, $timemessage);//kann weg ??
			// UDE-HACK end
