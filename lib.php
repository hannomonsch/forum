...
function forum_add_instance($forum, $mform = null) {
    ...
    if ($forum->type == 'single') {  // Create related discussion.
		...
		//UDE-HACK
        global $USER;
    	if (forum_is_anonymous($forum->id) && !has_capability('moodle/course:manageactivities', context_course::instance($forum->course), $USER->id, false)) {
			/* get user-ID of Anonymous user */
			$discussion->userid = get_config(null, 'forum_anonuser_id');
		} else {
...
function forum_update_instance($forum, $mform) {
    ...
    if ($forum->type == 'single') {  // Update related discussion and post.
		...
		if (forum_is_anonymous($forum->id) && !has_capability('moodle/course:manageactivities', context_course::instance($forum->course) , $USER->id, false)) {
			/* get user-ID of Anonymous user */
			$post->userid = get_config(null, 'forum_anonuser_id');
		} else {
...
function forum_add_discussion($discussion, $mform=null, $unused=null, $userid=null) {
    ...
    if (is_null($userid)) {
        //$userid = $USER->id;
		// UDE-HACK
		if (forum_is_anonymous($discussion->forum) && !has_capability('moodle/course:manageactivities', context_course::instance($discussion->course) , $USER->id, false)) {
			/* get user-ID of Anonymous user */
			$userid = get_config(null, 'forum_anonuser_id');
		} else {
			$userid = $USER->id; // übernehmen
		}
		// UDE-HACK end
    }
...
function forum_post_subscription($fromform, $forum, $discussion) {
    ...
    // UDE-HACK
	if (isset($fromform->discussionsubscribe)) {
...
// UDE-HACK
function forum_is_anonymous($forumid) {
	global $DB;

	if ($forum = $DB->get_record('forum', array('id' => $forumid))) {
		return (bool)$forum->isanonymous_ude;
	}

	return false;
}

function forum_get_coursemodule_info($cm) {
	global $DB;
	...
	if ($forum = $DB->get_record('forum', array('id' => $cm->instance))) {
		...
		if ($cm->showdescription) {
			$info->content = format_module_intro('forum', $forum, $cm->id, false);
		}

		// Override default plugin icon
		$info->iconcomponent = 'mod_forum';
		if (!forum_is_anonymous($cm->instance)) {
			$info->icon = 'icon';
		} else {
			$info->icon = 'icon_anon';
		}
