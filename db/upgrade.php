...
function xmldb_forum_upgrade($oldversion) {
    ...
    if ($oldversion < 2015061600) {
    	...
    	xmldb_forum_install(); // create anonymous user if it does not exist yet
    
    	$table = new xmldb_table('forum');
    	$field = new xmldb_field('isanonymous_ude', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, '0', null);
    
    	if (!$dbman->field_exists($table, $field)) {
    		$dbman->add_field($table, $field);
    	}
    
    	upgrade_mod_savepoint(true, 2015061600, 'forum');
    }
    // UDE-HACK end
